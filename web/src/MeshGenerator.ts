...
class MeshGenerator {
    ...
    public static GenerateMesh2(grid: Grid2d): [THREE.Mesh, [number, number][]] {
            const vertices: number[] = [];
            const triangles: number[] = [];
            ...
            for (let y = 0; y < grid.gridSize.y - 1; y++)
            {
                for (let x = 0; x < grid.gridSize.x - 1; x++)
                {
                    if (x <= grid.gridSize.x - 1 || y <= grid.gridSize.y - 1)
                    {
                        ...
                        // Find index in lookup table
                        let triIndex = 0;
                        if (cubeCorners[0].z < Settings.SURFACE_CUTOFF) triIndex |= 1;
                        if (cubeCorners[2].z < Settings.SURFACE_CUTOFF) triIndex |= 4;
                        if (cubeCorners[3].z < Settings.SURFACE_CUTOFF) triIndex |= 8;
                        if (cubeCorners[1].z < Settings.SURFACE_CUTOFF) triIndex |= 2;

                        // Create verticies
                        let doneWithBorder = false;
                        for (let i = 0; LookupTables.Square.TRIANGULATION_TABLE_EDGES[triIndex][i] != -1; i += 3)
                        {
                            {
                                if (LookupTables.Square.TRIANGULATION_TABLE_EDGES[triIndex] [i] == -2)
                                {
                                    i++;
                                    doneWithBorder = true;
                                }

                                const vertex = LookupTables.Square.TRIANGULATION_TABLE_EDGES[triIndex][i];
                                if (vertex != -1)
                                {
                                    // a b and c represent the points of triangle and 1 and 0 are the edge points to interpolate between for smoothing
                                    let a0 = LookupTables.Square.IN_BETWEEN_POINTS_A[LookupTables.Square.TRIANGULATION_TABLE_EDGES[triIndex][i]];
                                    let a1 = LookupTables.Square.IN_BETWEEN_POINTS_B[LookupTables.Square.TRIANGULATION_TABLE_EDGES[triIndex][i]];

                                    let b0 = LookupTables.Square.IN_BETWEEN_POINTS_A[
                                        LookupTables.Square.TRIANGULATION_TABLE_EDGES[triIndex][i + 1]];
                                    let b1 = LookupTables.Square.IN_BETWEEN_POINTS_B[
                                        LookupTables.Square.TRIANGULATION_TABLE_EDGES[triIndex][i + 1]];

                                    let c0 = LookupTables.Square.IN_BETWEEN_POINTS_A[
                                        LookupTables.Square.TRIANGULATION_TABLE_EDGES[triIndex][i + 2]];
                                    let c1 = LookupTables.Square.IN_BETWEEN_POINTS_B[
                                        LookupTables.Square.TRIANGULATION_TABLE_EDGES[triIndex][i + 2]];

                                    const newTri: Triangle2 = {};

                                    newTri.a = MeshGenerator.InterpolateVerts(cubeCorners[a0], cubeCorners[a1]);
                                    newTri.b = MeshGenerator.InterpolateVerts(cubeCorners[b0], cubeCorners[b1]);
                                    newTri.c = MeshGenerator.InterpolateVerts(cubeCorners[c0], cubeCorners[c1]);
    ...
    private static InterpolateVerts(v1: Vector3, v2: Vector3): Vector2
        {
            ...
            return new Vector2(v1.x, v1.y).add((new Vector2(v2.x, v2.y).sub(new Vector2(v1.x, v1.y))).multiplyScalar(t))
